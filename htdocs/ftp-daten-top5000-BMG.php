...
function fwritecsv_BMG($handle, $fields, $delimiter = "\t") {
...
function get_bmg_last_update_zeitkey($lastupdatefile){
...
function update_bmg_last_update_zeitkey($lastupdatefile, $zeitkey){
...
if (MDB2::isError($db)) {
	...
	$par['message'] = array('subject'=>"Error FTP-Daten BMG Top 5000",
...
$lastupdatefile	= $nfs_root_dir."BMGRIGHTS_TOP5000_PHYS.txt";
$last_time = get_bmg_last_update_zeitkey($lastupdatefile);


//Debug only don't forget to comment this again
//$zeit = 201501;
//$last_time = 201450;
...
if(($last_time && $last_time < $zeit) || !$last_time){
	...
	echo "seems we have new week PHYS data. Jahr:$jahr4 Woche:$woche \n "; 
	...
	$sql = "with
		fact_query AS(
			SELECT
				...
				row_number() over(order by sum(bwg_menge) DESC) index_
			FROM
				stamm_gesamt,
				region,
				bewegung_W,
	...
	if (MDB2::isError($rs)) {
	    ...
	    while($arr= $rs->fetchRow(MDB2_FETCHMODE_ASSOC)) {
	        if ($arr) {            
	            ...
	            $data[$ct]['stamm_ag_code'] 	= $arr['stamm_ag_code'];//Tonträger-Code
	            ...
	            $data[$ct]['voe_meng'] 		= $arr['voe_meng'];//Units über alle verkauften Wochen des Artikels
	        }
	        $ct++;
	    }
	    $rs->free();
	...
	echo "Schreibe phys. BMG-File...\n";
	$top5000_phys = "top5000_phys_".$jahr2.'KW'.$woche.".csv";
	$file = fopen($nfs_root_dir.'temp/'.$top5000_phys,"w");
	foreach ($data as $key => $value) {
		fwritecsv_BMG($file,$value);
	}
	fclose($file);
	//upload to FTP
	...
	if ( ! $ftp->put($target_path_de . '/BMGRIGHTS/' . $top5000_phys,
	    	$nfs_root_dir.'temp/'.$top5000_phys, FTP_BINARY)){
	  	echo "failed to upload: " . $top5000_phys . "\n";
	  	$is_error = true;
	  }
	echo "Updating $lastupdatefile ...\n";
	  if(!update_bmg_last_update_zeitkey($lastupdatefile, $zeit)){
	      echo "WARNING: failed to update last-update zeitkey ($zeit).\n";
	  }
	//email benachrichtigung
	if($is_error == true) {
	        $par['empfaenger'] = $reports_error_recipients;
	        $par['message'] = array('subject'=>"(BMG RIGHTS) Error FTP-PHYS Daten Wochen $top5000_phys",'body_txt'=>"Bei Generierung FTP-Daten (WOCHEN PHYS Daten, BMG Rights, $top5000_phys) Fehler aufgetreten\n");

	    } else {
	        $par['empfaenger'] = $reports_normal_recipients;
	        $par['message'] = array('subject'=>"(BMG RIGHTS) Erfolg FTP-PHYS Daten Wochen $top5000_phys",'body_txt'=>"FTP-Daten (WOCHEN PHYS Daten, BMG Rights, $top5000_phys ) generiert und verteilt.\n");
	    }
	    sendEMail($par);
...
$lastupdatefile	= $nfs_root_dir."BMGRIGHTS_TOP5000_DWN.txt";
$last_time = get_bmg_last_update_zeitkey($lastupdatefile);
...
if(($last_time && $last_time < $zeit) || !$last_time){
	...
	echo "Schreibe digi. BMG-File...\n";
	$top5000_digi = "top5000_digi_".$jahr2.'KW'.$woche.".csv";
	...
	$ftp->connect($ftp_host);
	$ftp->login($ftp_username, $ftp_password);
	$ftp_path = $target_path_de . '/BMGRIGHTS/' . $top5000_digi;
	...
	if($is_error == true) {
	        $par['empfaenger'] = $reports_error_recipients;
	        $par['message'] = array('subject'=>"(BMG RIGHTS) Error FTP-DWN Daten Wochen $top5000_digi",'body_txt'=>"Bei Generierung FTP-Daten (WOCHEN DWN Daten, BMG Rights, $top5000_digi) Fehler aufgetreten\n");
		
	    } else {
	        $par['empfaenger'] = $reports_normal_recipients;
	        $par['message'] = array('subject'=>"(BMG RIGHTS) Erfolg FTP-DWN Daten Wochen $top5000_digi",'body_txt'=>"FTP-Daten (WOCHEN DWN Daten, BMG Rights, $top5000_digi ) generiert und verteilt.\n");
	    }
	    sendEMail($par);
