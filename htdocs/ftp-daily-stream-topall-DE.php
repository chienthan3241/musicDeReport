...
function get_uni_single_key_array(){
	global $db;
	$result =array();
...
function get_uni_single_last_update_zeit_key($lastupdatefile){
	$r = array();
...
function update_uni_single_last_update_zeit_key($lastupdatefile,$arr){
...
$history_zeitkey_arr = array();
$current_zeitkey_arr = array();
$history_zeitkey_arr = get_uni_single_last_update_zeit_key($lastupdatefile);
$current_zeitkey_arr = get_uni_single_key_array();
$current_zeitkey_arr = array_reverse($current_zeitkey_arr);

foreach($current_zeitkey_arr as $zeit_with_num){
	if(!in_array($zeit_with_num, $history_zeitkey_arr)){
...
if($is_error){
    ...
    $par['message'] = array('subject'=>"Daten Daily Top all Single DE (".$zeit.")",'body_txt'=>$mail_body);
    sendEMail($par,$nfs_root_dir . 'stream/'.$file_name.'.txt');
}
}  
}
update_uni_single_last_update_zeit_key($lastupdatefile, $current_zeitkey_arr);
?>