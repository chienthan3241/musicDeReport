...
function report_error_groove_at(){
    if($is_error == true) {
...
function get_groove_at_last_update_zeitkey_month($lastupdatefile){
...
function update_groove_at_last_update_zeitkey_month($lastupdatefile, $zeitkey){
...
$lastupdatefile = $nfs_root_dir . 'monatsdata_last_update_groove_AT.txt';
...
if(substr($zeit,-2,2) != ( date('n') == '1' ? '13' : date('n') ) - 1){
  ...
  $last_time = get_groove_at_last_update_zeitkey_month($lastupdatefile);
...
ORDER BY
  ...
  if(!get_groove_at_last_update_zeitkey_month($lastupdatefile, $zeit)){
...
if($is_error == true) {
    $par['empfaenger'] = $reports_error_recipients;
    $par['message'] = array('subject'=>"(Exasol) Error GROOVE Monthly FTP- MonatsDaten Downloads AT",'body_txt'=>"there was an error by reports generation.\n");
  } else {
    $par['empfaenger'] = $reports_normal_recipients;
    $par['message'] = array('subject'=>"(Exasol) Erfolg GROOVE Monthly FTP-MonatsDaten Downloads AT",'body_txt'=>"FTP-Daten generiert und verteilt.\n");
  }
  sendEMail($par);
?>
