...
$dsn = $exasol_dsn; //'oci8://'.$ora_user.':'.$ora_pass.'@'.$ora_db_name;
$db = MDB2::connect($dsn);
if (MDB2::isError($db)) {
	die ($db->getMessage());
}
...
if(!$emif = fopen($nfs_root_dir . "temp/GERSX".substr($jahr,3).$woche."_new.SDF","w")) {
	$is_error=true;
} else {
    if( ! isset($daten)){
        $is_error = true;
...
if($is_error) {
	...
	$par['message'] = array('subject'=>"Error EMI-ALBEN-Datei",'body_txt'=>"(Exasol) Bei Generierung EMI-Datei Fehler aufgetreten!\n");
} else {	
	$par['empfaenger'] = $emi_reports_normal_recipients;
	$par['message'] = array('subject'=>"EMI-ALBEN-Datei",'body_txt'=>"(Exasol) EMI-ALBEN-Datei generiert, siehe Anhang.\n");
}
sendEMail($par,$nfs_root_dir."temp/GERSX".substr($jahr,3).$woche."_new.SDF");
unlink($nfs_root_dir . "temp/GERSX".substr($jahr,3).$woche."_new.SDF");
unset($daten);
?>
