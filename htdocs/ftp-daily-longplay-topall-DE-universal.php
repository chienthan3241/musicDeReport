...
function get_uni_LP_key_array(){
	global $db;
	$result =array();
	$qry = "select zeit_key || zeit_num as key from (
select zeit_key, sum(zeit_num) as zeit_num from (
select zeit_key, zeit_typeflag, 1 as zeit_num  from zeitraum_gui where zeit_einheit = 'T' AND
			zeit_landid = '1054' AND
			zeit_appid = 12 and zeit_typeflag in ('P','D')
		
	)  group by zeit_key) order by zeit_key desc limit 15";
	$rs = $db->query($qry);
...
function get_uni_LP_last_update_zeitkey($lastupdatefile){
    $r = array();
	if(is_readable($lastupdatefile) && filesize($lastupdatefile)!=0){
		$r = unserialize(file_get_contents($lastupdatefile));
	}else{
		echo "can not read $lastupdatefile file";
		die();
	}
	return $r;
}

function get_uni_LP_week_num($zeit,$type){
	global $db;
	$result='';
...
function update_uni_LP_last_update_zeitkey($lastupdatefile,$arr){
    $recursiv_zk = serialize($arr);
	$recursiv_hl = fopen($lastupdatefile,"w");
...
//===============================================================
$lastupdatefile = $nfs_root_dir . '/tageslongplay_uni_last_update.txt';

$history_LP_zeitkey_arr = array();
$current_LP_zeitkey_arr = array();
$history_LP_zeitkey_arr = get_uni_LP_last_update_zeitkey($lastupdatefile);
$current_LP_zeitkey_arr = get_uni_LP_key_array();
$current_LP_zeitkey_arr = array_reverse($current_LP_zeitkey_arr);

foreach($current_LP_zeitkey_arr as $zeit_with_num){
	if(!in_array($zeit_with_num, $history_LP_zeitkey_arr)){
	$zeit = substr($zeit_with_num, 0, 8);
	echo "generating tages Longplay data for $zeit ...\n";
	$jahr4 = substr($zeit,0,4);
    $monate2 = substr($zeit,-4,2);
    $tag2 = substr($zeit,-2,2);
    $current_week_num_p = get_uni_LP_week_num($zeit,'P');
    $current_week_num_d = get_uni_LP_week_num($zeit,'D');
    $current_week_num_s = get_uni_LP_week_num($zeit,'S');
    $mail_body = '';
    $udata = array();
    $sql = "
      with
...
//write to file
$file_name = 'LP-'.date('Y-m-d-H-i-s');
if(($file = fopen($nfs_root_dir . 'longplay/'.$file_name.'.txt', 'w')) === FALSE) {
    $is_error = true;
...
if($is_error){
    $par['empfaenger'] = $reports_error_recipients;
    $par['message'] = array('subject'=>"Error Universal Daten Daten Daily Top Longplay DE",'body_txt'=>$mail_body);
    sendEMail($par);
}else{
	$mail_body = ' ';
    $par['empfaenger'] = $email_delivery;
    $par['message'] = array('subject'=>"Daten Daily Top Longplay DE (".$zeit.")",'body_txt'=>$mail_body);
    sendEMail($par,$nfs_root_dir . 'longplay/'.$file_name.'.txt');
}
    
}
}
update_uni_LP_last_update_zeitkey($lastupdatefile, $current_LP_zeitkey_arr);
?>