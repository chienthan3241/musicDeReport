...
echo "\n\nSchreibe Daten.\n";
if(!$emif = fopen($nfs_root_dir . "temp/GERSC".substr($jahr,3).$woche."_new.SDF","w")) {
	$is_error=true;
} else {
...
if($is_error) {
	...
	$par['message'] = array('subject'=>"Error EMI-SINGLE-Datei",'body_txt'=>"(Exasol) Bei Generierung EMI-Datei Fehler aufgetreten!\n");
} else {
	$par['empfaenger'] = $emi_reports_normal_recipients;
	$par['message'] = array('subject'=>"EMI-SINGLE-Datei",'body_txt'=>"(Exasol) EMI-SINGLE-Datei generiert, siehe Anhang.\n");
}
sendEMail($par,$nfs_root_dir . "temp/GERSC".substr($jahr,3).$woche."_new.SDF");
unlink($nfs_root_dir . "temp/GERSC".substr($jahr,3).$woche."_new.SDF");
